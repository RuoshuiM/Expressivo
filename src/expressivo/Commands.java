 ...
 * 
 * <p>
 * PS3 instructions: this is a required class. You MUST NOT change its name or
 * package or the names or type signatures of existing methods. You MUST NOT add
 * fields, constructors, or instance methods. You may, however, add additional
 * static methods, or strengthen the specs of existing methods.
 */
public class Commands {
     ...
     *         variable v that appears in both the expression and the environment
     *         with its value, environment.get(v). If there are no variables left in
     *         this expression after substitution, it must be evaluated to a single
     *         number. Additional simplifications to the expression may be done at
     *         the implementor's discretion.
     * @throws IllegalArgumentException if the expression is invalid
     */
    public static String simplify(String expression, Map<String, Double> environment) {
        Expression simplified = Expression.parse(expression).simplify(environment);
//      Expression.visitAll(simplified, "--"); // for debugging
